...
class MGRSCoordConverter {
    ...
    private static final long[][] upsConstants = {{LETTER_A, LETTER_J, LETTER_Z, LETTER_Z, 800000, 800000}, {LETTER_B, LETTER_A, LETTER_R, LETTER_Z, 2000000, 800000}, {LETTER_Y, LETTER_J, LETTER_Z, LETTER_P, 800000, 1300000}, {LETTER_Z, LETTER_A, LETTER_J, LETTER_P, 2000000, 1300000}};
    ...
    private class MGRSComponents {
        ...
        public MGRSComponents(int zone, int latitudeBand, int squareLetter1, int squareLetter2, double easting, double northing, int precision) {
            this.zone = zone;
        ...
        if (MGRS == null) {
            ...
            if (error_code == MGRS_NO_ERROR) {
                if ((MGRS.latitudeBand == LETTER_X) && ((MGRS.zone == 32) || (MGRS.zone == 34) || (MGRS.zone == 36))) {
                    ...
                    if (error_code == MGRS_NO_ERROR) {
                        grid_northing = (double) (MGRS.squareLetter2) * ONEHT;  //   smithjl  commented out + false_northing;
                        ...
                        if (error_code == MGRS_NO_ERROR) {
                            ...
                            try {
                                ...
                                if (error_code == MGRS_NO_ERROR) {
                                    if (!(((south - DEG_TO_RAD / divisor) <= latitude) && (latitude <= (north + DEG_TO_RAD / divisor)))) {
    ...
    public long convertGeodeticToMGRS(double latitude, double longitude, int precision) {
        ...
        if (error_code == MGRS_NO_ERROR) {
            if ((latitude < MIN_UTM_LAT) || (latitude > MAX_UTM_LAT)) {
                // TODO: polar
                try {
                    UPSCoord UPS = UPSCoord.fromLatLon(Math.toDegrees(latitude), Math.toDegrees(longitude), globe);
                    error_code |= convertUPSToMGRS(UPS.getHemisphere(), UPS.getEasting(), UPS.getNorthing(), precision);
                } catch (Exception e) {
                    error_code = MGRS_UPS_ERROR;
                }
            } else {
                try {
                    UTMCoord UTM = UTMCoord.fromLatLon(Math.toDegrees(latitude), Math.toDegrees(longitude), globe);
                    error_code |= convertUTMToMGRS(UTM.getZone(), latitude, UTM.getEasting(), UTM.getNorthing(), precision);
                } catch (Exception e) {
                    error_code = MGRS_UTM_ERROR;
                }
            }
